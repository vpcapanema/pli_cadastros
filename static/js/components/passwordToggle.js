...
class PasswordToggle {
  ...
  addToggleToField(field) {
    ...
    const existingButton = document.querySelector(
      `#togglePassword, #toggle${field.id.charAt(0).toUpperCase() + field.id.slice(1)}, #toggle_${field.id}, [aria-describedby="${field.id}"]`
    );
    if (existingButton) {
    ...
    const container = field.closest(
      '.form-floating, .input-group, .form-group, .mb-3, .password-field-container'
    );
    if (!container) {
  ...
  createToggleButton(field, container) {
    ...
    button.id = toggleId;
    button.className =
      'btn btn-link position-absolute top-50 end-0 translate-middle-y me-2 p-1 password-toggle-btn';
