...
class IntelligentSessionManager {
    ...
    async init() {
        ...
        try {
            ...
            if (!Auth.isAuthenticated()) {
                console.log('[INTELLIGENT SESSION] Usuário não autenticado - sistema não será inicializado');
                return;
            }
    ...
    async loadSessionData() {
        try {
            ...
            if (!Auth.isAuthenticated()) {
                console.log('[INTELLIGENT SESSION] Usuário não autenticado - não é possível carregar dados da sessão');
    ...
    async unregisterWindow() {
        try {
            navigator.sendBeacon('/api/session/unregister-window', JSON.stringify({
                windowId: this.windowId,
                timestamp: Date.now()
            }));
        } catch (error) {
...
// Instância global
window.IntelligentSessionManager = window.IntelligentSessionManager || new IntelligentSessionManager();
