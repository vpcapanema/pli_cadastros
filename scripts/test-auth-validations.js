...
async function testarValidacoesAutenticacao() {
    ...
    try {
        ...
        usuarios.rows.forEach(user => {
            ...
            console.log(`   - ${user.username || user.email_institucional}: ${user.status} | Ativo: ${user.ativo} | Email Verificado: ${user.email_institucional_verificado} | Pode Logar: ${statusAuth ? '✅' : '❌'}`);
        });
        ...
        const cenarios = [
            ...
            { status: 'AGUARDANDO_APROVACAO', ativo: true, email_verificado: true, resultado: '❌ USUARIO_NAO_APROVADO' },
            { status: 'APROVADO', ativo: false, email_verificado: true, resultado: '❌ USUARIO_INATIVO' },
            { status: 'APROVADO', ativo: true, email_verificado: false, resultado: '❌ EMAIL_NAO_VERIFICADO' },
            { status: 'REJEITADO', ativo: false, email_verificado: false, resultado: '❌ MULTIPLAS_VIOLACOES' }
        ];

        cenarios.forEach((cenario, index) => {
            console.log(`   Cenário ${index + 1}: Status=${cenario.status}, Ativo=${cenario.ativo}, Email=${cenario.email_verificado} → ${cenario.resultado}`);
        });
...
if (require.main === module) {
    testarValidacoesAutenticacao().then(() => {
