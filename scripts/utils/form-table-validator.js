...
function compareTableAndForm(tableFields, formFields) {
  ...
  const missingInForm = tableFieldNames.filter(field => 
    !formFieldNames.includes(field) && 
    !['id', 'created_at', 'updated_at', 'data_criacao', 'data_atualizacao', 'data_exclusao'].includes(field)
  );
  
  // Campos que existem no formulário mas não na tabela
  const missingInTable = formFieldNames.filter(field => 
