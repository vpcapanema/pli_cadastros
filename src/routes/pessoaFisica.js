  ...
  try {
    ...
    if (testTable.rows[0].count == 0) {
      return res
        ...
        .json({
          error:
            'Estrutura de tabela ausente (cadastro.pessoa_fisica). Provisionar banco antes de usar a API.',
        });
    }
    const sql = `
      SELECT 
    ...
    res
      .status(500)
      .json({ error: 'Erro interno ao buscar pessoas físicas', details: error.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const checkCpf = await query('SELECT id FROM cadastro.pessoa_fisica WHERE cpf = $1', [
      cpfFormatado,
    ]);
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const orgaoExpeditorFormatado =
      orgao_expeditor && orgao_expeditor.trim() !== '' ? orgao_expeditor : null;
    ...
    const telefonePrincipalFormatado =
      telefone_principal && telefone_principal.trim() !== ''
        ? formatPhone(telefone_principal)
        : null;
    const telefoneSecundarioFormatado =
      telefone_secundario && telefone_secundario.trim() !== ''
        ? formatPhone(telefone_secundario)
        : null;
    ...
    if (cpfFormatado) {
      const checkCpf = await query(
        'SELECT id FROM cadastro.pessoa_fisica WHERE cpf = $1 AND id != $2',
        [cpfFormatado, id]
      );
      if (checkCpf.rows.length > 0) {
