...
class SecurityLogMonitor extends EventEmitter {
    ...
    async sendAlert(alertType, details) {
        ...
        if (this.lastAlerts.has(alertKey)) {
            ...
            if (now - lastAlert < 300000) { // 5 minutos
                return;
            }
        }
    ...
    getLogPath(logType) {
        switch (logType) {
            case 'security': return this.logPath;
            case 'audit': return this.auditLogPath;
            case 'error': return this.errorLogPath;
            default: return this.logPath;
        }
    }
}
