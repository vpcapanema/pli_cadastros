...
${this.analysisResults.attackAttempts.length > 0 ? 
    ...
    this.analysisResults.attackAttempts.map(attack => 
...
${topIPs.length > 0 ? 
    '| IP | Eventos | Status |\n|----|---------|---------|\n' +
    topIPs.map(([ip, count]) => 
        `| ${ip} | ${count} | ${this.getIPStatus(ip, count)} |`
    ).join('\n')
...
${this.analysisResults.authEvents.length > 0 ?
    this.analysisResults.authEvents.map(auth => {
...
*Relatório gerado automaticamente pelo Sistema de Monitoramento SIGMA-PLI*`;
    ...
    generateSecurityRecommendations() {
        ...
        if (this.analysisResults.attackAttempts.length > 0) {
            recommendations.push('🚨 **CRÍTICO:** Foram detectadas tentativas de ataque. Revisar logs detalhados e considerar bloqueio de IPs suspeitos.');
        }
        
        if (this.analysisResults.errorEvents.length > 10) {
            recommendations.push('⚠️ **ATENÇÃO:** Alta taxa de erros detectada. Verificar estabilidade do sistema.');
        }
        
        if (this.analysisResults.ipStatistics.size > 50) {
            recommendations.push('📊 **INFO:** Muitos IPs únicos acessando o sistema. Considerar implementar rate limiting mais restritivo.');
        }
