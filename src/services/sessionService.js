...
const SessionService = {
  ...
  async registrarLogout(tokenHash, motivo = 'LOGOUT_MANUAL') {
    try {
      ...
      if (result.rows.length > 0) {
        console.log(
          `[SESSION] Logout registrado: ${result.rows[0].session_id} - Motivo: ${motivo}`
        );
      }
  ...
  async registrarJanela(userId, sessionId, windowId, url, ip, timestamp) {
    try {
      ...
      console.log(
        `[SESSION] Janela ${windowId} registrada - Total: ${totalWindows}, Main: ${isMainWindow}`
      );
  ...
  async verificarEMarcarLogoutSeNecessario(sessionId) {
    try {
      ...
      if (janelasAtivas === 0) {
        console.log(
          `[SESSION] Todas as janelas fechadas - realizando logout automático da sessão ${sessionId}`
        );
        ...
        if (sessionResult.rows.length > 0) {
          await this.registrarLogout(sessionResult.rows[0].token_jwt_hash, 'ALL_WINDOWS_CLOSED');
        }
      }
    } catch (error) {
