...
function hasOAuth2() {
  return ['OAUTH2_CLIENT_ID','OAUTH2_CLIENT_SECRET','OAUTH2_REFRESH_TOKEN','SMTP_USER'].every(k => process.env[k]);
}
function validateEmailProvider() {
  if (hasSmtpBasic() || hasSendgrid() || hasOAuth2()) return [];
  return ['Config provedor email ausente: defina SMTP_* completo, ou SENDGRID_API_KEY, ou OAUTH2_* + SMTP_USER'];
}
...
function validateNumeric(vars) {
  return vars.filter(v => process.env[v] && isNaN(Number(process.env[v]))).map(v => `Variável ${v} deve ser numérica`);
}
function validateEnv(options = {}) {
  const strict = options.strict !== undefined ? options.strict : process.env.NODE_ENV === 'production';
  ...
  const problems = [];
  if (missingBase.length) problems.push(`Variáveis obrigatórias ausentes: ${missingBase.join(', ')}`);
