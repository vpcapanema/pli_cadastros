...
const globalErrorHandler = (error, req, res, next) => {
  ...
  if (status >= 500) {
    ...
    logSecurityEvent('SERVER_ERROR', 'high', error.message, {
      stack: error.stack,
      status,
      type
    }, req);
    ...
    if (status === 401 || status === 403) {
      logSecurityEvent('ACCESS_DENIED', 'medium', error.message, {
        status,
        type
      }, req);
    }
...
const validateJSON = (error, req, res, next) => {
  if (error instanceof SyntaxError && error.status === 400 && 'body' in error) {
    logSecurityEvent('INVALID_JSON', 'low', 'JSON mal formado recebido', {
      error: error.message,
      body: req.body
    }, req);
...
const requestTimeout = (timeout = 30000) => {
  return (req, res, next) => {
    res.setTimeout(timeout, () => {
      logSecurityEvent('REQUEST_TIMEOUT', 'medium', 'Timeout de requisição', {
        timeout,
        url: req.originalUrl,
        method: req.method
      }, req);
...
const detectBruteForce = (req, res, next) => {
  ...
  if (req.path.includes('/auth/') && res.statusCode >= 400) {
    ...
    if (attempts.count >= 5) {
      logSecurityEvent('BRUTE_FORCE', 'high', 'Possível ataque de força bruta detectado', {
        ip,
