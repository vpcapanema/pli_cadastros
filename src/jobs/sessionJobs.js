 ...
 */
const limpezaSessoesExpiradas = cron.schedule(
  '*/30 * * * *',
  async () => {
    try {
 ...
 */
const limpezaRegistrosAntigos = cron.schedule(
  '0 2 * * *',
  async () => {
    try {
 ...
 */
const invalidarSessoesInativas = cron.schedule(
  '*/15 * * * *',
  async () => {
    try {
      ...
      if (sessoes_inativadas > 0) {
        console.log(
          `[CRON] ${sessoes_inativadas} sessão(ões) marcada(s) como INATIVA por inatividade`
        );
      }
    } catch (error) {
 ...
 */
const estatisticasDiarias = cron.schedule(
  '59 23 * * *',
  async () => {
    try {
