// moved from inline <script>
        ...
        function populateForm(userData) {
            ...
            document.getElementById('dataAdmissao').value = userData.data_criacao ? userData.data_criacao.split('T')[0] : '';

            // Informações da conta
            document.getElementById('userId').textContent = userData.id || '-';
            document.getElementById('dataCriacao').textContent = userData.data_criacao ? formatDate(userData.data_criacao) : '-';
            document.getElementById('ultimoAcesso').textContent = userData.data_ultimo_login ? formatDate(userData.data_ultimo_login) : '-';
...
document.getElementById("auto_evt_6bcf612e").addEventListener("click", function(event) {
  var __result;
  try {
    __result = (function(){ toggleEditMode() }).call(this, event);
...
document.getElementById("auto_evt_8cff6972").addEventListener("click", function(event) {
  var __result;
  try {
    __result = (function(){ cancelEdit() }).call(this, event);
  } catch(e) { console.error(e); }
  if (__result === false) { event.preventDefault(); event.stopPropagation(); }
});
